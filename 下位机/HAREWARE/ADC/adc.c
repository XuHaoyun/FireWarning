...
void Adc_Init(void)
{
	...
	// DMA配置
	DMA_InitStructure.DMA_DIR = DMA_DIR_PeripheralSRC;   // 外设传信号给DMA
	DMA_InitStructure.DMA_BufferSize = 3;                // 传输大小
...
int getSmogValue( void ){
	...
	returnSmog = (float)(AD_Value[1] / 40.96);
	returnSmog = returnSmog>99 ? 99 : returnSmog;
...
int getFlameValue( void ){
	...
	// 电压越近越大
	voltage = (float)AD_Value[0] / 4096 * 5.0;
	
	distance = voltage * 10;
	distance = (50 - distance) * 2;
